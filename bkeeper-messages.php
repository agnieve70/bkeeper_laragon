            ...
            <div class="row px-5 mt-5">
                <div class="col-md-3 bg-primary p-2">
                    <ul class="list-group">
                        ...
                        OR id IN (SELECT client_id FROM bkeepers WHERE bkeeper_id = " . $_SESSION['user_id'] . ")
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                <a href="http://localhost/bkeeper_laragon/bkeeper-messages.php?to_id=' . $row['id'] . '&to_name=' . $row['name'] . '" class="text-decoration-none">
                                <h6>' . $row['name'] . '</h6>
                                </a>
                            </li>';
                                }
                ...
                <div class="col-md-9 px-5 h-96">
                    ...
                    if (isset($_GET['to_id']) && isset($_GET['to_name'])) {
                        ...
                        $sql2 = "SELECT `id`, `message`, `date`, (SELECT name from users WHERE users.id = `from`) as `from_name`,
                            (SELECT name from users WHERE users.id = `to`) as `to_name` from message WHERE `from` = " . $_GET['to_id'] ." AND `to` = ".$_SESSION['user_id']
                            . ' OR `from` = '.$_SESSION['user_id']. ' AND `to` = '.$_GET['to_id'];
                    ...
                    if (isset($_POST['send_message'])) {
                        ...
                        $sql2 = "INSERT INTO notification (`title`, `link`, `user_id`, `date`, `code`)
                                VALUES ('Message From Bkeeper', 
                                        'http://localhost/bkeeper_laragon/admin-messages.php?to_id=" . $_SESSION['user_id'] . "&to_name=" . $_SESSION['name'] . "',
                                        '" . $to_id . "',
                        ...
                        if (mysqli_query($conn, $sql)) {
                            echo '<script>alert("Message Successfully Send"); window.location.href="http://localhost/bkeeper_laragon/bkeeper-messages.php?to_id=' . $to_id . '&to_name=' . $to_name . '" </script>';
                        } else {
